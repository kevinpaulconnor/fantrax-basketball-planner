module.exports.matchupDates = [
    {
        opponent: "Joel",
        start: new Date("2021-10-19").toISOString(),
        end: new Date("2021-10-24").toISOString(),
    },
    {
        opponent: "Paul",
        start: new Date("2021-10-25").toISOString(),
        end: new Date("2021-10-31").toISOString(),
    },
    {
        opponent: "James",
        start: new Date("2021-11-01").toISOString(),
        end: new Date("2021-11-07").toISOString(),
    },
    {
        opponent: "Jason",
        start: new Date("2021-11-08").toISOString(),
        end: new Date("2021-11-14").toISOString(),
    },
    {
        opponent: "Taylor",
        start: new Date("2021-11-15").toISOString(),
        end: new Date("2021-11-21").toISOString(),
    },
    {
        opponent: "CharLZ",
        start: new Date("2021-11-22").toISOString(),
        end: new Date("2021-11-28").toISOString(),
    },
    {
        opponent: "Pat",
        start: new Date("2021-11-29").toISOString(),
        end: new Date("2021-12-05").toISOString(),
    },
    {
        opponent: "Mike",
        start: new Date("2021-12-06").toISOString(),
        end: new Date("2021-12-12").toISOString(),
    },
    {
        opponent: "Justin",
        start: new Date("2021-12-13").toISOString(),
        end: new Date("2021-12-19").toISOString(),
    },
    {
        opponent: "Meyon",
        start: new Date("2021-12-20").toISOString(),
        end: new Date("2021-12-26").toISOString(),
    },
    {
        opponent: "SyKo",
        start: new Date("2021-12-27").toISOString(),
        end: new Date("2022-01-02").toISOString(),
    },
    {
        opponent: "Mike",
        start: new Date("2022-01-03").toISOString(),
        end: new Date("2022-01-09").toISOString(),
    },
    {
        opponent: "Syko",
        start: new Date("2022-01-10").toISOString(),
        end: new Date("2022-01-16").toISOString(),
    },
    {
        opponent: "James",
        start: new Date("2022-01-17").toISOString(),
        end: new Date("2022-01-23").toISOString(),
    },
    {
        opponent: "Jason",
        start: new Date("2022-01-24").toISOString(),
        end: new Date("2022-01-30").toISOString(),
    },
    {
        opponent: "Justin",
        start: new Date("2022-01-31").toISOString(),
        end: new Date("2022-02-06").toISOString(),
    },
    {
        opponent: "ChrLZ",
        start: new Date("2022-02-07").toISOString(),
        end: new Date("2022-02-13").toISOString(),
    },
    {
        opponent: "Pat",
        start: new Date("2022-02-14").toISOString(),
        end: new Date("2022-02-27").toISOString(),
        // probably has a different gp limit?
    },
    // colbys playoffs begin
    {
        opponent: "Playoffs Round 1",
        start: new Date("2022-02-28").toISOString(),
        end: new Date("2022-03-06").toISOString(),
    },
    {
        opponent: "Playoffs Round 2",
        start: new Date("2022-03-07").toISOString(),
        end: new Date("2022-03-20").toISOString(),
        matchupLength: 14,
        // probably has a different gp limit?
    },
    {
        opponent: "Playoffs Round 3",
        start: new Date("2022-03-21").toISOString(),
        end: new Date("2022-04-03").toISOString(),
        matchupLength: 14,
        // probably has a different gp limit?
    },
    ];